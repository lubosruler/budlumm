...
- RPC API: Relayer modeli netleştiği için (permissionless) RPC uç noktaları
  ...
  sorgulama uçları dahil. **Tur 14, Faz 5 — B.U.D. storage RPC yüzeyi
...
- **B.U.D. (Tur 14):** `ConsensusKind::StorageAttestation(StorageDomainParams)`
...
- `src/registry/role.rs` — generic `RoleId` (hard-coded enum değil). Bilinen
  ...
  `STORAGE_OPERATOR` (RoleId 5) **Tur 14, Faz 1**: permissionless, stake ile
...
- `src/domain/storage_params.rs` — **Tur 14, Faz 1 (B.U.D.)**:
...
- `src/domain/storage_deal.rs` — **Tur 14, Faz 5 (B.U.D. deal/challenge
  ...
  admin/pause hook'u YOK — data-sovereignty kuralı (Tur 14.5 plan §0.5).
- `src/storage/content_id.rs` — **Tur 14, Faz 2 (B.U.D. content
  addressing)**: `ContentId` (32-byte domain-tagged SHA-256).
  `of_subrange` byte-range hash (interim) — gerçek proof DEĞİL.
- `src/storage/manifest.rs` — **Tur 14, Faz 2**: `ContentManifest` +
