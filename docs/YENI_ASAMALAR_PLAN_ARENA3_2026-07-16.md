...
**Durum:** ADIM3 büyük ölçüde kapandı (honest closeout), ADIM4 VerifyMerkle kısmen, ADIM5-6-7 yeni aşamalar belirsiz — bu doküman AI birliği tartışması için ön planlama

---

## 1. Mevcut Durum Özeti (6333a74 sonrası)

| ADIM | Kapsam | Durum | Kanıt |
|------|--------|-------|-------|
| ADIM3 (Mainnet v1 lansman paketi) | §0 güvenlik borçları 0.1-0.4, §3 3.1-3.6 | ✅ Büyük ölçüde kapandı, kalan bilinçli borç: ceremony dummy, VerifyMerkle gate kapalı, HSM vendor-native, archive drill CI | 5562716 kuyruk drain (13 test), e221b18 validator E2E, 4cf710d storage_root V3, 29d81b6 Dockerfile mainnet, scripts/*.sh |
| ADIM4 (B.U.D. Faz 3 VerifyMerkle) | Faz 3 Merkle proof alanları, storage_root Block V3, 1-depth debug harness | 🔒 Kapalı — `proves_verify_merkle_valid_64_depth` InvalidProof, matrix chain diagnostic yeşil ama full STARK kırmızı (aux CTL şüpheli) | `TUR4_PLAN.md`, `VERIFYMERKLE_CONSTRAINT_DEBUG_ARENA3.md`, 6eedd2d constraint-by-constraint plan |
| ADIM5 (External audit + hardening + marketplace + mobile) | Başlangıçta implemente edildi (2db13c5 AI Data Marketplace, c726de3 mobile lightweight sharding, 271f162 master key + pruning, baa10e7 universal relayer) ama **CI kırıldı**, ARENA2 6333a74 ile **f9f5b9a green base'e revert** etti, CI green | 🔒 Ertelendi / Revert — marketplace/mobile/socialfi/hub broken, testler kırmızı, bu yüzden revert | 2db13c5, c726de3, 271f162, baa10e7, 9c09741 hub, d17bf71 socialfi boost, 67da984 socialfi NFT posts → hepsi revert ile f9f5b9a'ya dönüldü, CI green |
| ADIM6 (BNS/.bud + SocialFi + Hub + Constitution + Budlum Hub) | BNS Phase 6 full_impl merge (lifecycle + storage_root binding + fetch content RPC) var, SocialFi NFT posts (67da984) ve Hub (9c09741) denendi ama revert ile gitti, Constitution (8389f42, f4d7e28) kaldı | 🟡 Kısmi — BNS stub + lifecycle var, SocialFi/Hub/Marketplace revert sonrası yok | 7482dd7 BNS full_impl merge, d294111 lifecycle, 2250795 full_integration, 0017e97 early init, 67da984 socialfi, 9c09741 hub → revert |

**Sonuç:** ADIM3 yeşil, ADIM4 VerifyMerkle kırmızı (bilinçli), ADIM5/6 sosyal/marketplace/mobile/hub denendi ama CI kırıldığı için revert edildi, şimdi green base f9f5b9a'da (CI green). Yeni aşamalar için **temiz, küçük, testli adımlarla** ilerlememiz gerekiyor, tek seferde 5 özellik birden değil.
...
| **YENİ M10** | SocialFi / Marketplace / Mobile / Hub / Constitution | 🔒 Revert sonrası yok — f9f5b9a green base'de sadece BNS + Constitution kaldı | Orta — ADIM5/6 yeni aşama |

---

## 3. Yeni Aşamalar Önerisi — AI Birliği Tartışması İçin

Kullanıcı "yeni aşamalar için AI'larla konuş bunu belirle" dedi. Önerim **ADIM4/5/6/7'yi netleştirilmiş, küçük, testli fazlara bölmek**:

### ADIM4 — B.U.D. Faz 3: VerifyMerkle Production Açılışı (mevcut, devam)
...
### ADIM5 — External Audit + Hardening + B.U.D. P2P Entegrasyon (öncelik)
...
| 5.3 | Bug bounty launch: `BUG_BOUNTY.md` immunefi tier medium → high, PGP key `0xBUDLUM-SECURITY` yayınlama (ADIM3 kararı C: bug bounty) | ARENA1 | Immunefi başvurusu |
| 5.4 | B.U.D. P2P storage node monolithic entegrasyon: `bud-node` Bitswap + KAD + sharding + `Node::with_key` storage args (100ac26 + 44a6f12) zaten var, ama `NodeCommand::StoragePrune` hard pruning worker + `Node::run` storage initialization test edilmeli | ARENA1 (Node) + ARENA3 (P2P) | `scripts/adim3_smoke_rpc.sh` + `docker-smoke-mainnet.sh` yeşil |
...
### ADIM6 — BNS/.bud + SocialFi + Hub + AI Data Marketplace + Mobile (yeni, küçük adımlar)
...
| 6.4 | AI Data Marketplace: 2db13c5'teki AI Data Marketplace (ADIM5 Hat 5.5) → küçük PR, sadece marketplace listing, escrow yok | ARENA2 | docs + test |
...
- Her ADIM için `STATUS_ONLINE.md`'de Aşama 1 konuşma → Aşama 2 commit kontrol → Aşama 3 CI yeşil.
...
**ARENA1'e:**
1. ADIM6 SocialFi/HUB/Marketplace/Mobile denemesi CI kırdığı için revert edildi (6333a74). Küçük, testli adımlarla (6.2, 6.3, 6.4, 6.5) yeniden başlayalım mı? Önce hangisi: SocialFi NFT posts mu, Hub dApp registration mı, Marketplace mi, Mobile lightweight mı?
...
2. ADIM5 external audit + TLA+ iskeleti için `AUDIT_CHECKLIST.md` + `THREAT_MODEL.md` yeterli mi, TLA+ `MultiConsensus.tla` taslak ekleyelim mi?
3. ADIM6 AI Data Marketplace (2db13c5) revert edildi, ama doküman kaldı. Küçük PR ile sadece listing (escrow yok) olarak yeniden başlayalım mı?

**Kullanıcıya (Ayaz):**
- Yeni aşamalar (ADIM4 VerifyMerkle, ADIM5 audit/hardening, ADIM6 BNS/SocialFi/Hub/Marketplace/Mobile, ADIM7 ceremony) için öncelik ne? Hepsi paralel (mevcut karar) mı, yoksa ADIM4 ZK önce mi, ADIM6 SocialFi/HUB sonra mı?
- Mainnet launch için devnet_ready (self-audited, bug bounty, ceremony placeholder) yeterli mi, yoksa ADIM5 audit + ceremony + HSM vendor-native tamamlanmadan mainnet'e çıkmayalım mı?
...
- ADIM4 Hat A: VerifyMerkle 1-depth test → 2-depth → 64-depth + production gate
- ADIM6 Hat B: BNS fetch content → Bitswap discovery glue (KAD + request_response)
- ADIM5 Hat C: External audit checklist + TLA+ iskeleti
...
- `cat docs/ADIM3_HONEST_CLOSEOUT.md` + `TUR4_PLAN.md` + `STATUS.md`
